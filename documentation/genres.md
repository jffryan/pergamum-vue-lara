...
- **`GenreController::show` builds on `BookListing`, the library's listing query.** It used to carry its own join-and-`GROUP BY` shape, which ordered on `MIN(authors.last_name)` — the alphabetically first author, not the primary one the row renders — and left-joined `read_instances` raw, outside the reach of `ReadInstance`'s `BelongsToCurrentUser` scope. Both are gone. If you add a sort dimension, add it to `BookListing::query()` and both pages get it.
...
Books are ordered exactly as the library orders them: by the name the *primary* author (lowest `author_ordinal`) files under, with `Author::sortNameExpression()` handling mononyms, books with no author last, and `books.book_id` breaking ties so pages stay disjoint. The endpoint takes no `?sort=` yet — the detail page has no sort UI, and wiring one is passing the two request inputs into `BookListing::sort()`. Default page size is 20; pass `?limit=` to override. The SPA links into this view via `{ name: 'genres.show', params: { id: genre_id } }` (note: ID, not slug); `BookTableRow` already does this for each book's first two genres.
