...
- **Services**: `GenreService` (`app/Services/GenreService.php`) owns the name rules — `normalize`, `findConflict`, `create`, `rename`, `delete`, `merge`. It raises `GenreNameConflictException` and `GenreInUseException` from `app/Services/Exceptions/`, both carrying a `reasonCode` the controller renders. `BookService::getBooksList` is reused to format the per-genre book payload. Genre *attachment* during book create/update does **not** go through `GenreService` yet — it still lives on `BookController::handleGenres` / `updateGenres` and `NewBookController::handleGenres` (see `books.md` and `new-book-creation.md`).
...
- **Stores**: `GenreStore` (`stores/GenreStore.js`) holds `allGenres` (the cached index payload) and `currentPage` (used by `GenresView`'s client-side pagination, *not* by `GenreView` which paginates server-side via the route query string). It also owns `fetchAllGenres({ force })` and the four mutation actions.
...
- **Three divergent ingest paths, all keyed by raw `name`.** Genres are created/attached from three places, each with a slightly different input shape:
  ...
  All three call `Genre::firstOrCreate(['name' => $name])`. **They do not go through `GenreService`** — consolidating them behind a `GenreService::attachByName()` is tracked in `/feature-plans/genres.md`, and was deliberately left out of the admin CRUD work because it touches the book create/edit pipeline.
...
- **`genres.name` is not unique at the DB level.** `firstOrCreate` and `GenreService::findConflict` are the only dedupe. Concurrent inserts (or any future bulk-import path) can still produce duplicate names. A unique index is the planned end state but can only land after the merge tool has cleaned out what's already in the database — see `/feature-plans/genre-management.md`.
...
Two independent paths. Genres are still created/attached implicitly by `POST /create-book` (new-book wizard, via `NewBookController::handleGenres`) and `POST /books` (`BookController::handleGenres`), and re-synced by `PATCH /books/{id}` (`BookController::updateGenres`) — see `books.md` and `new-book-creation.md`. The direct CRUD API below is what `/admin/genres` drives.

Names are normalized before validation on create and rename: trimmed, with internal whitespace collapsed. Casing is **not** touched — display casing is a UI concern and the collation makes it irrelevant for matching. A whitespace-only name is therefore a `required` failure, not a genre named four spaces.
