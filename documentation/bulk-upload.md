...
1. **Book**: `BookMatcher::find(title, authors)` — an existing book whose title slugs the same *and* shares at least one author with the row — else `BookCreator::create`, which files a same-title different-author book under its own slug (`ariel-rodo`). Title on existing books is left alone. A row whose author list merely adds a foreword writer or translator to a known book lands on that book; a row with the same title and no author in common is a new book. This used to match on the title slug alone, which is how two different *Ariel*s became one.
...
- **Book and author slugs come from `App\Support\Slugger`,** the same helper the SPA's creation paths use — 60-character cap, truncated at a hyphen boundary. An author slug match means "same author"; a book needs a title match *and* an author in common (`BookMatcher`), and a new book's slug comes from `BookCreator::create` so a second same-title book gets `title-surname` rather than colliding. `lists.slug` and `formats.slug` are different entities and derive from raw `Str::slug`.
