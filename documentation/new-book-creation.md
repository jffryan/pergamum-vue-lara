...
- **`NewVersionsInput` validates `audio_runtime` against the wrong format name.** The select stores the `Format` object with `name: "Audiobook"` (capital A), but `validateVersion` checks `format?.name === "audio"`. The audio runtime input is shown (template uses `=== "Audiobook"`) but `audio_runtime` is *never* validated as required. A user can submit an audiobook with empty runtime; backend will store `null`. Fix the validator to match the format name actually in use.
