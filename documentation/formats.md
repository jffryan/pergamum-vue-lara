...
A `Format` is a flat lookup record (`format_id`, `name`, optional `slug`, plus two capability flags) describing the medium of a `Version` — Audiobook, Physical, Ebook, etc. Unlike `Author` and `Genre`, formats are never created as a side effect of book creation: they are pre-existing config that the SPA bootstraps from `GET /api/config/formats` and that the new-book / edit-book / add-version forms select from. New formats are added explicitly via the admin surface (`POST /formats`). Books are reachable by format at `/formats/:slug`, which reuses the books index endpoint with a `?format=` filter.

**A format declares what it is measured in.** `expects_page_count` and `expects_audio_runtime` are the answer to "does a version of this format have pages / a runtime", and every consumer — the create and edit endpoints, the bulk importer, and all four book forms — reads them instead of matching on the format's name or id. Adding a medium with different length semantics is a row in `formats`, not a branch in `prepareVersions`. The canonical rows live in `database/seeders/FormatSeeder.php`.
...
- **Routes** (`routes/api.php`, all under `auth:sanctum`):
  - `GET /config/formats` → `ConfigController::getFormats` — lightweight `[{format_id, name, expects_page_count, expects_audio_runtime}, ...]` payload the SPA caches in `ConfigStore`. Lives under `/config/` because formats are treated as bootstrap config, not as a queryable taxonomy. The capability flags are part of the contract: the book forms decide which length inputs to render from them.
  - `POST /formats` → `FormatController::store` — admin-style create. Validates `name` as `required|string|max:255|unique:formats,name`, derives `slug` via `Str::slug($name)`, and accepts optional `expects_page_count` / `expects_audio_runtime` booleans that default to `true` / `false` — a print format, which is what most new formats are. Returns the full row with 201.
...
- **Services**: none. Format logic is inline in the two controllers above, in `BookController::lengthFieldsFor` (used by both `prepareVersions` and `updateVersions`), and in `BulkImportService::validateRow` — all three of which ask the format what it expects rather than what it is called.
- **Models**: `Format` (`format_id` PK, fillable `['name', 'slug', 'expects_page_count', 'expects_audio_runtime']`, both flags cast to `boolean`) declares both `versions(): HasMany` and a convenience `books(): BelongsToMany` that traverses through the `versions` table (`belongsToMany(Book, 'versions', 'format_id', 'book_id')`). `Version::format(): BelongsTo` is the inverse and is the load shape every consumer expects (`versions.format`). `expectedLengthFields()` returns the flags keyed by column (`['page_count' => bool, 'audio_runtime' => bool]`) so a caller can iterate rather than branch; `expects('page_count')` is the single-field form.
...
- **Migrations**: `2023_09_09_000004_create_formats_table.php` defines just `format_id` / `name` / nullable `slug` / timestamps. `name` is **not** unique at the DB level (the controller-side `unique:formats,name` rule is the only dedupe). `slug` is nullable, not unique, and not indexed. `2026_08_02_000002_add_capability_flags_to_formats_table.php` adds `expects_page_count` (default `true`) and `expects_audio_runtime` (default `false`), backfills the Audiobook row to the inverse, and makes `versions.page_count` nullable so a format that carries no page count can say so. The `versions` table (`2023_09_09_000005_create_versions_table.php`) holds the `format_id` FK that joins back here.
- **Seeder**: `database/seeders/FormatSeeder.php` (called from `DatabaseSeeder`) holds the canonical rows — Physical, Audiobook, Pirated, Ebook, Graphic Novel — with their capability flags. It `updateOrCreate`s on name, so it both seeds a fresh database and repairs one whose flags predate the migration. Bulk import fails every row when no format matches, so this has to run before any import into a fresh database.
...
- **Two payload shapes for the same row, reconciled in the store.** `GET /config/formats` returns the four-key projection; `POST /formats` returns the full Eloquent row including `slug` and timestamps. `ConfigStore.createFormat` used to push the latter onto the array `setFormats` fills with the former, leaving the cache heterogeneous for the rest of the session. It now refetches instead, because the forms read capability flags off whatever is in that array and a shape mismatch would silently hide a length input.
...
- **A format's capability is authoritative; its name and id are not.** `prepareVersions` used to branch on `$format->name == 'Audiobook'` / `'Paper'`, `NewVersionsInput.vue` and `EditBookView.vue` matched the name, and `BookCreateEditForm.vue` / `AddVersionView.vue` / `AddReadHistoryView.vue` hardcoded `format_id === 2` — five conventions for one question. All of them now read `expects_page_count` / `expects_audio_runtime`. Two consequences worth knowing: the `'Paper'` branch had **never matched anything** (the format is called "Physical"), so every non-audio format silently fell through to the audio branch; and a value the format doesn't expect is now **nulled on write** rather than passed through, so re-formatting an audiobook as physical clears its runtime instead of leaving an orphan that statistics still sum. The frontend reads the flags through `resources/js/utils/formats.js` — `expects(format, field)` when a component already holds the format object, `formatExpects(formats, formatId, field)` when it only has an id from a `<select>`.
- **An unresolved format expects nothing.** `utils/formats.js` returns `false` for a null/unknown format rather than guessing, so a form that renders before `ConfigStore.checkForFormats()` resolves shows no length input rather than the wrong one. The matching rule on the validation side is that a field the format doesn't carry always passes — there is no input on screen to clear the error with.
- **"Discarded" is not a format, and must not become one.** A copy we no longer own is recorded on the `Version` (`is_discarded` / `discarded_at`), not by swapping `format_id` — see `/documentation/books.md`. Format means *medium*; ownership state is orthogonal to it, and folding one into the other would erase the medium of every discarded copy (and send it down the wrong branch of `prepareVersions`, which string-matches on format name). The same reasoning applies to any future "on loan" / "wishlist" style state.
