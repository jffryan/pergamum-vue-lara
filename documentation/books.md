...
- **Slugs are generated from title at create/update time** (lowercase, non-alphanumerics stripped, spaces → hyphens, 50-char cap). Two books with colliding slugs after normalization will be treated as the same book by `createOrGetBook` — there is no uniqueness suffix. Slug normalization logic is duplicated across `createOrGetBook`, `updateBook`, `updateAuthors`, and `handleAuthors`; if you need to change it, change all four.
