...
- **Models**: `ReadInstance` (PK `read_instance_id`, fillable `user_id`, `book_id`, `version_id`, `date_read`, `rating`). Date mutator returns `Y-m-d`; rating is doubled on write and halved on read, so the model speaks the display scale. See `books.md` for both.
...
- **`date_read` is normalized at the request boundary.** `App\Http\Requests\Concerns\NormalizesReadDates` accepts `Y-m-d`, `m/d/Y` and `n/j/Y` and stores `Y-m-d`; anything else passes through for the `date` rule to reject. Controllers no longer parse dates — `Carbon::createFromFormat('Y-m-d', …)` *threw* on the `m/d/Y` value the edit form round-trips, which was a 500. Note Carbon throws rather than returning `false` on a format mismatch, and parses leniently besides, so the concern round-trips the parse to decide whether a format actually matched.
...
- **Version-belongs-to-book is enforced server-side.** `ReadInstance::booted()` registers a `saving` listener that throws `\DomainException` if `version_id`'s `Version.book_id` doesn't match the row's `book_id` — this is the safety net that protects every write path (controllers, bulk import, future code). `addReadInstance` does the same check up front and returns `422 { reason_code: 'version_book_mismatch' }` so callers see a structured error instead of a 500.
