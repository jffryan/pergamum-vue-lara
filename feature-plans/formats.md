...
- ~~**`NewVersionsInput.validateVersion` checks `format?.name === 'audio'`.**~~ Fixed. Validation now gates on the format's capability flags, and the related bug it was hiding is fixed too: `page_count` was required unconditionally, including for audiobooks whose input isn't rendered — an unclearable error.
- ~~**Three different ways to identify "Audiobook".**~~ Fixed — there were five, and all of them now read `expects_page_count` / `expects_audio_runtime`. Renaming a format or reseeding with different ids no longer changes any behavior in the book forms or the importer. The `?format=` browse filter is the one place a rename still matters; see the Correctness bug above.
...
- ~~**Heterogeneous shape in `ConfigStore.books.formats`** after `createFormat`.~~ Fixed — `createFormat` refetches rather than pushing the POST response. It stopped being cosmetic once the forms began reading capability flags off that array.
...
3. ~~**Replace string / id matching with capability flags.**~~ Done. `formats.expects_page_count` / `expects_audio_runtime` are the single answer, read by `BookController::lengthFieldsFor` (create *and* edit), `BulkImportService::validateRow`, and all four book forms via `resources/js/utils/formats.js`. `versions.page_count` became nullable in the same migration. Two things the work turned up that the plan didn't anticipate: the `'Paper'` branch had never matched anything (the format is called **Physical**), so every non-audio format fell through to the audio branch; and the validation gate in `NewVersionsInput` was requiring `page_count` unconditionally, including for audiobooks whose input isn't rendered. `tests/Feature/Books/VersionLengthFieldsTest.php` and `resources/js/tests/utils/formats.test.js` pin it. See `/documentation/formats.md`.
...
5. **Build PATCH / DELETE for formats.** `PATCH /formats/{id}` for renames (re-derive slug) and for flipping the capability flags, which are currently settable only at create time — a format seeded with the wrong pair can only be fixed in SQL or by re-running `FormatSeeder`. `DELETE /formats/{id}` only when no `Version` references the format — return 409 otherwise. Surface both in the admin UI as edit / delete buttons on `FormatsList`. Renames are now safe for the book forms (item 3 removed every name match), but still break `BookController::index`'s `?format=` filter until item 1 lands.
...
8. ~~**Seed the canonical formats.**~~ Done — `database/seeders/FormatSeeder.php`, called from `DatabaseSeeder`, holds Physical / Audiobook / Pirated / Ebook / Graphic Novel with their capability flags. It `updateOrCreate`s on name so it also repairs a database whose flags predate item 3. Insert order matches the existing ids, but nothing depends on that any more.
9. ~~**Invalidate `ConfigStore.books.formats` after admin create.**~~ Done as part of item 3 — `createFormat` now calls `setFormats()` instead of pushing the POST response, because the forms read capability flags off that array and a heterogeneous entry would silently hide a length input.
