...
5. **Build a format-merge tool.** `POST /admin/formats/{keep_id}/merge/{remove_id}` re-points `versions.format_id` and deletes the loser. Required before any unique-slug index in item 7 can land cleanly. Mirror the shipped genre merge — `GenreService::merge` and `/documentation/genres.md` — rather than re-deriving it; in particular, attach-the-difference-then-delete rather than a blind `UPDATE`, since `versions.format_id` has its own version of the same duplicate hazard.
