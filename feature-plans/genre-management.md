...
- **The admin genre table has no sort and no pagination.** It renders every genre in API order, filtered by the shared `filterGenres` from `utils/genreList.js`. The sort and letter-grouping helpers next to it are deliberately unused here — every row carries actions, so section headings get in the way. Fine at the current genre count; revisit when it stops being.
...
6. **Give the admin table a sort.** The helpers already exist and are tested (`sortGenres` in `utils/genreList.js`); what's missing is a header control and a decision about whether an admin sorting by book count wants the same tie-breaking a browsing user does. Only worth doing when the genre count makes the API-ordered list unusable.
