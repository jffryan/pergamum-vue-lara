...
9. **Add a "list type" enum and a `ListTypeHandler` registry** keyed by type slug, each defining what fields apply, what stats to compute, and what default ordering to use. This is the seam CLAUDE.md asks for. Now that shelving is `locations` (`/feature-plans/locations.md`) and the shelf lists are pending deletion, every remaining list is genuinely one type and the next one added is genuinely different — this item stops being speculative the day those lists are deleted. Don't build it until the second type is on the roadmap, but plan for it.
