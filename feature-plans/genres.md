...
- ~~**`GenreController::show` does a redundant join + groupBy.**~~ Fixed: `show` builds on `App\Support\BookListing`, the same correlated-subquery listing query `BookController::index` uses. No joins, no `GROUP BY`, and the ordering is the primary author's filing name rather than `MIN(last_name)` — the old expression ranked a collaboration by whichever of its authors sorted first alphabetically, which is not the author the row renders.
- ~~**`books.readInstances` is not user-scoped on the genre detail page.**~~ Fixed by `App\Models\Scopes\BelongsToCurrentUser` on `ReadInstance`, which scopes the eager load in `GenreController::show`. Pinned by `tests/Feature/UserScoping/TaxonomyScopingTest`. The raw `read_instances` join that sat outside the scope's reach is gone with the rest of the old query shape.
- **`GenresView` loads and renders the full genre list.** Deliberate at the current scale (~140), and the filter and sort are in-memory. There's no server-side search or pagination on the index endpoint, so this stops being viable somewhere in the low thousands — the grid, not the fetch, is what gives out first.
...
- **`GenreStore` is a thin cache.** `allGenres` and the mutations, nothing else — no `currentGenre`, no per-genre book cache. Anyone extending the detail page will need to flesh out the store rather than discover existing wiring (mirrors `AuthorsStore`).

### Frontend & UX

- **Sort options on `GenresView` are name and book count only.** No "recently added", no "most read" (which would need user-scoped reads anyway). Neither is in the index payload, so both need a backend change first.
...
- **Renaming is admin-only.** `GenresView` links to `/admin/genres`, but `GenreView` and the book edit form don't — so noticing a bad genre on a book and fixing it are still two separate journeys.
...
1. **Add Feature tests for the read endpoints.** `GenreController::index`'s ordering rule (including the numeric-prefix REGEXP) is still unpinned; `show`'s ordering, eager-load order and pagination tiebreak are covered by `GenreBooksOrderingTest`, its `?limit=` cap is not. The three *attachment* paths are now covered by `GenreIngestTest`, and the admin CRUD and merge surface by the rest of `tests/Feature/Genres/`.
...
4. ~~**Drop the redundant `read_instances` join and groupBy in `GenreController::show`.**~~ Done — the whole query was replaced by `App\Support\BookListing::query()`, which is join-free.
5. **Add server-side search to `GET /genres`.** `?q=` filter against `name`, paginated. Both search boxes share `filterGenres` from `utils/genreList.js`, so this is one call site to swap for a debounced fetch — the letter-jump rail on `GenresView` is the piece that has to go when the full list stops being client-side.
