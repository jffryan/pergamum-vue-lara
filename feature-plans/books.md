...
- **`GenreController::show` still joins `read_instances` without a user filter at the join level.** `BookController::index` no longer does — the library listing was rebuilt on correlated subqueries when sorting landed, and a subquery built from an Eloquent model carries `BelongsToCurrentUser` where a raw `leftJoin` never could. `GenreController::show` kept the old shape and is now the last instance of it; the same rewrite applies.
...
7. **Give `GenreController::show` the subquery treatment.** Last holder of the un-scoped `leftJoin('read_instances', …)`; `BookController::libraryQuery()` is the worked example.
