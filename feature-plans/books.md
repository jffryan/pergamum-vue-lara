...
11. **Give the discard filter a scope rather than a private controller method.** `BookController::applyDiscardedFilter` is the only implementation; the moment a second listing endpoint needs it (year-browse, format browse, lists) it should move to a `Book::scopeOnShelf()` / `scopeDiscarded()` pair so the "every version discarded" rule lives in one place. Rule of three isn't hit yet — one caller, plus `searchBooks` in the same file.
