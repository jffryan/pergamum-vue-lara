...
- **`books.slug` has no DB-level unique constraint.** `BulkImportService::resolveBook` is read-then-write; concurrent uploads of the same title could each pass the find-by-slug check and both create the book. Tracked alongside the broader slug-uniqueness work in `/feature-plans/books.md`. Land that constraint before running the actual restore so any pre-existing duplicates surface in a controlled migration rather than mid-restore.
