...
- **Services** in `app/Services/` (`BookService`, `AuthorService`, `BulkImportService`) hold business logic; controllers are thin and inject services via the constructor. Statistics are the exception: they live in `app/Statistics/` as a scope + metric registry rather than one service — see `/documentation/statistics.md`. Some domain logic also lives on models (e.g., date formatting).
